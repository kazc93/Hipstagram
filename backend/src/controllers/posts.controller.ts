...
// Si un hashtag falla, el post se guarda de todas formas (ROLLBACK parcial)
export const crearPost = async (req: any, res: Response) => {
  const id_usuario = req.usuario.id;
  const { descripcion } = req.body;
  // multer-s3 guarda la URL pública de S3 en req.file.location
  ...
  try {
    ...
    // ── Moderación: verificar hashtags prohibidos ────────────────────────────
...
export const eliminarPost = async (req: any, res: Response) => {
  ...
  try {
    ...
    // FK bloqueado: si hay votos o comentarios, retornar 409
